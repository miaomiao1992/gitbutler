...
///
/// This rejection is the last line of defense. Ideally, hunk dependency
/// detection would prevent the user from staging the change to the wrong
/// stack in the first place (see `project_hunk_dep_base_gap` for the
/// known gap there).
#[test]
fn commit_to_wrong_base_rejects_conflicting_file() -> Result<()> {
    ...
    assert!(
        ...
        "expected CherryPickMergeConflict for file.txt, got: {:?}",
